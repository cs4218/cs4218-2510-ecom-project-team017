...
describe("ForgotPassword Component", () => {
    ...
    describe("Form Input Handling - Equivalence Partitioning", () => {
        // Valid equivalence class
        test("should accept valid email input", () => {
            renderComponent();
        ...
        test("should accept valid text for security answer", () => {
            renderComponent();
        ...
        test("should accept valid password input", () => {
            renderComponent();
        ...
        test("should accept empty strings in inputs (HTML5 validation handles required)", () => {
            renderComponent();
            const emailInput = screen.getByPlaceholderText("Enter Your Email");
            const answerInput = screen.getByPlaceholderText("What is Your Favorite Sports?");
            const passwordInput = screen.getByPlaceholderText("Enter Your New Password");
    ...
    describe("Password Length - Boundary Value Analysis", () => {
        const fillFormExceptPassword = (password) => {
            const emailInput = screen.getByPlaceholderText("Enter Your Email");
            const answerInput = screen.getByPlaceholderText("What is Your Favorite Sports?");
            const passwordInput = screen.getByPlaceholderText("Enter Your New Password");

            fireEvent.change(emailInput, { target: { value: "test@example.com" } });
            fireEvent.change(answerInput, { target: { value: "Soccer" } });
            fireEvent.change(passwordInput, { target: { value: password } });
        };

        // Below minimum boundary (assuming 6 char minimum from hint)
        test("should accept password with 5 characters (boundary-1)", async () => {
            renderComponent();
            fillFormExceptPassword("pass5");

            const passwordInput = screen.getByPlaceholderText("Enter Your New Password");
            expect(passwordInput.value).toBe("pass5");
        });

        // At minimum boundary
        test("should accept password with exactly 6 characters (boundary)", async () => {
            renderComponent();
            fillFormExceptPassword("pass66");

            const passwordInput = screen.getByPlaceholderText("Enter Your New Password");
            expect(passwordInput.value).toBe("pass66");
        });

        // Above minimum boundary
        test("should accept password with 7 characters (boundary+1)", async () => {
            renderComponent();
            fillFormExceptPassword("pass777");

            const passwordInput = screen.getByPlaceholderText("Enter Your New Password");
            expect(passwordInput.value).toBe("pass777");
        });

        // Very long password (upper boundary test)
        test("should accept very long password (100 characters)", async () => {
            renderComponent();
            const longPassword = "a".repeat(100);
            fillFormExceptPassword(longPassword);
    ...
    describe("Email Format - Equivalence Partitioning", () => {
        ...
        test("should accept email with subdomain", () => {
            ...
            expect(emailInput.value).toBe("user@mail.example.com");
        });
        ...
        test("should accept email with numbers and special characters", () => {
            ...
            fireEvent.change(emailInput, { target: { value: "user.name+tag123@example.co.uk" } });
            expect(emailInput.value).toBe("user.name+tag123@example.co.uk");
        });
    });
